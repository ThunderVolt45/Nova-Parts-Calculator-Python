...
- 기존 Qt Designer 기반 GUI 구조와 파일 배치를 우선 유지합니다.
...
배포 대상은 `dist\NovaPartsCalculator.exe` 단일 실행 파일입니다. `build` 폴더 안의 파일은 임시 빌드 산출물이므로 실행하거나 배포하지 않습니다.
...
- 배포 파일 확인: `dist\NovaPartsCalculator.exe`
